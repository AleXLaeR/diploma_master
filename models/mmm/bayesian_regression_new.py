"""Bayesian regression MMM rewrite for task #9."""
...
MODEL_NAME_A = "MMM_Bayesian_Heuristic"
MODEL_NAME_B = "MMM_Bayesian_DDA"
...
def _build_model(
    ...
    """Construct Bayesian regression MMM model graph."""
    n_channels = x_media.shape[1]
    c_scale = float(np.mean(y)) / max(n_channels, 1)

    coords = {
        "channel": CHANNEL_NAMES,
        "exog": EXOG_COLUMNS,
        "time": np.arange(x_media.shape[0]),
    }
    ...
    with pm.Model(coords=coords) as model:
        intercept = pm.Normal("intercept", mu=float(np.mean(y)), sigma=max(float(np.std(y)), 1.0))
        ...
        if scenario == "B" and shapley_weights is not None:
            beta_mu = np.array([
                float(shapley_weights.get(channel, 1.0 / len(CHANNEL_NAMES))) * c_scale
                for channel in CHANNEL_NAMES
            ])
        ...
        mu = intercept + media_contrib + exog_contrib

        sigma_obs = pm.HalfCauchy("sigma_obs", beta=1.0)
        y_obs = pm.Data("y_obs", y)
        pm.Normal("Y_obs", mu=mu, sigma=sigma_obs, observed=y_obs, dims="time")
...
def _rhat_ok(trace: az.InferenceData) -> bool:
    ...
    summary = az.summary(trace, var_names=["beta", "K", "S", "intercept"], round_to=4)
...
def _posterior_outputs(
    ...
    """Compute posterior means for forecast, baseline and media contributions."""
    posterior = trace.posterior

    beta = posterior["beta"].values.reshape(-1, len(CHANNEL_NAMES))
    k = posterior["K"].values.reshape(-1, len(CHANNEL_NAMES))
    s = posterior["S"].values.reshape(-1, len(CHANNEL_NAMES))
    ...
    base_component = intercept[:, np.newaxis] + exog_contrib
    ...
    return {
        "ppc": ppc,
        "expected_mean": ppc.mean(axis=0),
        "base_mean": base_component.mean(axis=0),
        "media_weekly_channel_mean": media_channel.mean(axis=0),
...
def _fit_segment(
    ...
    y_train = train_df["total_net_revenue_usd"].to_numpy(dtype=float)
    ...
    posterior = _posterior_outputs(trace, x_media_holdout=x_media_holdout, x_exog_holdout=exog_holdout)
...
def _run_scenario(
    ...
    """Run one Bayesian prior scenario and persist outputs."""
...
def run(
    ...
    """Run Bayesian MMM scenarios A/B and persist eval + incr contributions."""
    ...
    if train_all.empty:
        logger.error("Bayesian MMM fold=%s: empty training data, skipping.", fold_id)
        return
