...
What to plot:
- On the left: square matrix where rows = origin state (channel or "Start"), columns = destination state (channel, "Conversion" or "Null"). Cell values = transition probability. Annotate each cell with the probability. Add a title annotation: total journeys analyzed and base conversion rate.
- On the right: removal effect bar plot for every paid channel.
...
# F7.1, F7.2 — Cohort Retention Curves & Survival Decay Curves.
What to plot:
- F7.1: X-axis = rebill number (period since acquisition, 1 through max horizon). Y-axis = retention rate. Three series: empirical observed retention, exponential curve fit, BdW fit. Show 2 cohorts for SUB_MONTLY and 1 for SUB_WEEKLY for some macro-region (e.g, LATAM) as a representative example.
- F7.2: X-axis = cohort age in months. Y-axis = active users ($S(t)$ output). Three series: empirical, exponential fit, BdW fit. Show 4 cohorts across different folds, holdout weeks AND regions.

The story: all three lines will be close at early periods (rebill 1–3). At rebill 4+, the exponential curve will diverge upward from empirical, overestimating retention. The BdW curve will track the empirical tail much more closely.
...
What to plot: X-axis = cohort age in months (1 through max horizon). Y-axis = RMSE. Two lines: exponential and BdW. The lines should start close and diverge - the crossing/divergence point is the visual thesis of this plot.

Data requirement: Per-cfold model predictions at multiple horizons.


# F9 — LTV Extrapolation Bias (small multiples per cohort).
What to plot: one panel per acquisition cohort (e.g., cohorts acquired in months 1-8 of your dataset). In each panel: observed cumulative revenue curve, exponential model extrapolation, BdW model extrapolation. Mark the train/holdout boundary with a vertical dashed line. The extrapolation zone is to the right of that line. Should be implemented for multiple distinct cohorts as subplots.
